...
CREATE VIEW vw_kpi_summary AS
    SELECT 
        ...
        ROUND(AVG(days_for_shipping_real - days_for_shipping_scheduled),
                2) AS avg_transit_variance_days,
        ROUND(STDDEV(days_for_shipping_real - days_for_shipping_scheduled),
                2) AS stddev_transit_variance,
        ROUND(SUM(sales), 2) AS total_sales,
        ROUND(SUM(order_profit_per_order), 2) AS total_profit,
        ROUND(100.0 * SUM(order_profit_per_order) / NULLIF(SUM(sales), 0),
                2) AS overall_margin_pct
    FROM
        fact_orders;
...
CREATE VIEW vw_shipping_mode_scorecard AS
    SELECT 
        ...
        COUNT(*) AS volume,
        ROUND(100.0 * SUM(CASE
                    WHEN f.days_for_shipping_real <= f.days_for_shipping_scheduled THEN 1
                    ELSE 0
                END) / COUNT(*),
                2) AS on_time_pct,
        ROUND(AVG(f.days_for_shipping_real), 2) AS avg_actual_days,
        ROUND(AVG(f.days_for_shipping_scheduled), 2) AS avg_scheduled_days,
        ROUND(AVG(f.days_for_shipping_real - f.days_for_shipping_scheduled),
                2) AS avg_delay_days,
        ROUND(100.0 * SUM(CASE
    ...
    FROM
        fact_orders f
            JOIN
        dim_shipping_mode sm ON sm.shipping_mode_key = f.shipping_mode_key
...
CREATE VIEW vw_market_performance AS
    SELECT 
        ...
        ROUND(100.0 * SUM(CASE
                    WHEN
                        f.days_for_shipping_real <= f.days_for_shipping_scheduled
        ...
        ROUND(SUM(f.order_profit_per_order), 2) AS total_profit,
        ROUND(100.0 * SUM(f.order_profit_per_order) / NULLIF(SUM(f.sales), 0),
                2) AS margin_pct
    FROM
        fact_orders f
            JOIN
        dim_geography g ON g.geography_key = f.order_geography_key
    GROUP BY g.market
    ORDER BY volume DESC;
...
CREATE VIEW vw_category_demand_monthly AS
    ...
    FROM
        ...
        dim_date d ON d.date_key = f.order_date_key
            JOIN
        dim_product p ON p.product_key = f.product_key
...
CREATE VIEW vw_top_problem_routes AS
    ...
    FROM
        fact_orders f
            JOIN
        dim_geography og ON og.geography_key = f.order_geography_key
            JOIN
        dim_geography cg ON cg.geography_key = f.customer_geography_key
...
CREATE VIEW vw_customer_segment_profitability AS
    SELECT 
        ...
        ROUND(100.0 * SUM(f.order_profit_per_order) / NULLIF(SUM(f.sales), 0),
                2) AS margin_pct,
        ROUND(SUM(f.sales) / NULLIF(COUNT(DISTINCT c.customer_id), 0),
    ...
    FROM
        fact_orders f
            JOIN
        dim_customer c ON c.customer_key = f.customer_key
